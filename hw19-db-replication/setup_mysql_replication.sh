...
docker exec mysql_slave sh -c "export MYSQL_PWD=111; mysql -u root -e 'SHOW SLAVE STATUS \G'"#!/bin/bash
...
priv_stmt='CREATE USER "mydb_slave_user"@"%" IDENTIFIED BY "mydb_slave_pwd"; 
           ...
           CREATE USER "mydb_slave2_user"@"%" IDENTIFIED BY "mydb_slave2_pwd"; 
           GRANT REPLICATION SLAVE ON *.* TO "mydb_slave2_user"@"%"; 
           FLUSH PRIVILEGES;'
