...
curl -u $RABBITMQ_USER:$RABBITMQ_PASS -X POST -H "Content-Type: application/json" \
  -d '{"properties":{}, "routing_key":"'$RABBITMQ_QUEUE'", "payload":"'$MESSAGE'", "payload_encoding":"string"}' \
  http://localhost:15672/api/exchanges/%2F/amq.default/publish

echo "Publishing message to Redis RDB..."
redis-cli -h localhost -p 6379 XADD stream_rdb * message "$MESSAGE"

echo "Publishing message to Redis AOF..."
redis-cli -h localhost -p 6380 XADD stream_aof * message "$MESSAGE"

echo "Message published successfully!"

# Pull script to retrieve messages
